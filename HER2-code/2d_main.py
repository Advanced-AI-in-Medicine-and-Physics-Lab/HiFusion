...
parser.add_argument('--root_path', type=str, default='data/',
                    help='data path')
parser.add_argument('--save_path', type=str, default='save/HER2_3d/test/',
...
class PerImageNormalize(object):
    def __call__(self, img_tensor):
        ...
        mean = torch.mean(img_tensor, dim=(1, 2))  # 按通道计算
...
if __name__ == "__main__":
    ...
    for fold_index, fold_split in enumerate(fold_splits):
        ...
        for sample_dir in os.listdir(label_root):
            
            if sample_dir not in fold_split:
                for sample in os.listdir(os.path.join(label_root, sample_dir)):
                    train_labels_path.append(os.path.join(label_root, sample_dir, sample)) # data/npy_information/sample_dir/sample.npy
                    
            else:
                for sample in os.listdir(os.path.join(label_root, sample_dir)):
                    val_labels_path.append(os.path.join(label_root, sample_dir, sample))
            

        train_dataloaders = create_dataloaders_for_each_file(train_labels_path, root=root, batch_size=batch_size, transform=img_transform, shuffle=True)
        test_dataloader = create_dataloaders_for_each_file(val_labels_path, root=root, batch_size=batch_size, transform=test_img_transform, shuffle=False)
        ...
        for epoch_num in tqdm(range(max_epoch), ncols=70):
            ...
            for file_name in file_names:
                ...
                for region_images, spot_images, labels in tmp_loader:
                    ...
                    with torch.autocast("cuda", enabled=True): 
                        ...
                        gene_pred_loss = mse_loss_fn(preds[-1], labels) #+ w*mse_loss_fn(extra_pred, labels.cuda())
            ...
            writer.add_scalar(f'loss/{fold_index+1}_loss', epoch_loss/epoch_iter, epoch_num)
            scheduler.step()
            ...
            if epoch_num % 1 == 0:
                ...
                with torch.no_grad():
                    ...
                    writer.add_scalar(f'validation/{fold_index+1}_mse', val_mse / total_samples, epoch_num)
                    ...
                    if val_mse / total_samples < best_mse:
                        ...
                        torch.save(model.state_dict(), os.path.join(save_path, f"model_{fold_index}.pth"))
                        print(f'Best model saved to {os.path.join(save_path, f"model_{fold_index}.pth")}')
    ...
    # np.save(os.path.join(save_path, 'validation_metrics.npy'),
