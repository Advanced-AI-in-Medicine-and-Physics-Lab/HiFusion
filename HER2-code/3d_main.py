...
if __name__ == "__main__":
    ...
    for fold_index, fold_split in enumerate(fold_splits):
        ...
        for sample in fold_split:
            train_labels_path = []
            val_labels_path = []
            for sample_layer in os.listdir(os.path.join(label_root, sample)):
                # train on 1st slide, test on the others
                if '1' in sample_layer:    
                    train_labels_path.append(os.path.join(label_root, sample, sample_layer))
                else:
                    val_labels_path.append(os.path.join(label_root, sample, sample_layer))
            ...
            for epoch_num in tqdm(range(max_epoch), ncols=70):
                ...
                file_names = list(train_dataloaders.keys()) # e.g. [A1]
                ...
                writer.add_scalar(f'loss/{sample}', epoch_loss/epoch_iter, iter_num)
                scheduler.step()
            ...
            print('Fold-{}-Sample-{} Training Completed! Time: {:0>2}:{:0>2}:{:0>2}\n'.format(fold_index+1,sample,h, m, s))
